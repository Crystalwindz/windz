...
void ThreadPool::Start(size_t thread_num) {
    ...
    for (size_t i = 0; i < thread_num; ++i) {
        threads_.emplace_back(MakeUnique<Thread>([this] {
            while (running_) {
...
void ThreadPool::AddTask(const Task &task) { queue_.Push(task); }

void ThreadPool::AddTask(Task &&task) { queue_.Push(std::move(task)); }

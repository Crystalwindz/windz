...
void TestBlockQueue() {
    ...
    for (int i = 0; i < 4; i++) {
        producer1.push_back(make_shared<Thread>([i] {
            for (int j = 0; j < 100; j++) {
                stringstream str;
        ...
        consumer1.push_back(make_shared<Thread>([i] {
            while (true) {
                stringstream str;
    ...
    // start produce
    for_each(producer1.begin(), producer1.end(), [](shared_ptr<Thread> &t) { t->Start(); });
    for_each(producer1.begin(), producer1.end(), [](shared_ptr<Thread> &t) { t->Join(); });
    ...
    for_each(consumer1.begin(), consumer1.end(), [](shared_ptr<Thread> &t) { t->Start(); });
...
void TestLimitQueue() {
    ...
    for (int i = 0; i < 4; i++) {
        producer2.push_back(make_shared<Thread>([i] {
            for (int j = 0; j < 10; j++) {
                stringstream str;
                str << "Producer " << i + 1 << " " << j + 1 << "th thing\n";
        ...
        consumer2.push_back(make_shared<Thread>([i] {
            while (true) {
                stringstream str;
                sleep(5);
    ...
    for_each(producer2.begin(), producer2.end(), [](shared_ptr<Thread> &t) { t->Start(); });

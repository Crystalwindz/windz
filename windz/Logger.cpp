...
class T {
  public:
    T(const char *str, size_t len) : str_(str), len_(len) { assert(strlen(str) == len); }
...
Logger::Logger(const char *filename, int line, LogLevel level, const char *func, int saved_errno)
    : stream_(), filename_(filename), line_(line), level_(level) {
...
Logger::Logger(const char *filename, int line) : Logger(filename, line, INFO) {}
...
void Logger::FormatTime() {
    ...
    int len = snprintf(t_timebuf, sizeof(t_timebuf), "%4d%02d%02d %02d:%02d:%02d.%06d ",
...
Logger::LogLevel Logger::Level() { return g_loglevel; }

void Logger::SetLevel(LogLevel level) { g_loglevel = level; }
